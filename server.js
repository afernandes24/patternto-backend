...
app.post('/api/generate-pdf', async (req, res) => {
  ...
  try {
    // ── Launch Chromium ───────────────────────────────────────────────────────
    // On Railway (persistent Linux container) we can use the system Chromium
    // if available, falling back to @sparticuz/chromium for the bundled binary.
    const executablePath =
      process.env.CHROMIUM_PATH ||          // override via env if desired
